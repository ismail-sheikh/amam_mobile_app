...
class WooCommerceService extends BaseServices {
  ...
  Future<List<Product>?> boostProducts({
    ...
    try {
      ...
      if (category != null && category != kEmptyCategoryID && category != '0') {
        elasticService.mustList.addMatch(
          key: 'categories.id',
          value: '$category',
        );
      }
      ...
      if (maxPrice != null && maxPrice > 0) {
        elasticService.mustList.addRange(
          key: 'price',
          gteValue: minPrice,
          lteValue: maxPrice,
        );
      ...
      elasticService.mustNotList
          .addMatch(key: 'catalog_visibility', value: 'hidden');
      ...
      if (attributeId != null) {
        ...
        if (termId.isNotEmpty) {
          elasticService.mustList.addFilter(
            keyFilter: 'terms',
      ...
      if (orderBy is String) {
        ...
        elasticService.addSort(key: orderBy, value: order ?? 'desc');
      }
      // If add this param to API, the result will not contain backorder products
      if (kAdvanceConfig.hideOutOfStock) {
        elasticService.mustList.addMatch(key: 'in_stock', value: true);
      }
  ...
  Future<List<Product>?> fetchProductsLayout({
    ...
    try {
      ...
      if (kAdvanceConfig.isCaching &&
          configCache != null &&
          (dataConfig['page'] == 1 || dataConfig['page'] == null)) {
        var obj;
        ...
        if (horizontalLayout != null) {
          obj = horizontalLayout.firstWhere(
              (o) =>
                  ...
                  _cachedLayout.contains(o['layout']) &&
        ...
        if (verticalLayout != null &&
            verticalLayout['layout'] == dataConfig['layout'] &&
            ((verticalLayout['category'] != null &&
                    verticalLayout['category'] == dataConfig['category']) ||
                (verticalLayout['tag'] != null &&
                    verticalLayout['tag'] == dataConfig['tag']))) {
      ...
      if (dataConfig.containsKey('advancedParams') &&
          dataConfig['advancedParams'] != null &&
          dataConfig['advancedParams'] is Map) {
        dataConfig['advancedParams']?.forEach((key, value) {
          endPoint += '&$key';
      ...
      if (dataConfig['category'] != null &&
        ...
        endPoint += "&category=${dataConfig["category"]}";
      }

      if (dataConfig['tag'] != null &&
          dataConfig['tag'].toString().isNotEmpty) {
        endPoint += "&tag=${dataConfig["tag"]}";
      }

      /// Add featured filter
      if (dataConfig.containsKey('featured') &&
          dataConfig['featured'] == true) {
        endPoint += "&featured=${dataConfig["featured"]}";
      }

      /// Add onSale filter
      if (dataConfig.containsKey('onSale') && dataConfig['onSale'] == true) {
        endPoint += "&on_sale=${dataConfig["onSale"]}";
      }

      if (dataConfig['minPrice'] != null &&
          dataConfig['maxPrice'] != null &&
          dataConfig['maxPrice'] > 0) {
        endPoint +=
            '&min_price=${(dataConfig['minPrice'] as double).toInt().toString()}&max_price=${(dataConfig['maxPrice'] as double).toInt().toString()}';
      }

      if (dataConfig['attribute'] != null) {
        endPoint += '&attribute=${dataConfig['attribute']}';
      }

      if (dataConfig['attributeId'] != null) {
        endPoint += '&attribute_term=${dataConfig['attributeId']}';
      }

      /// sort by
      if (dataConfig.containsKey('orderby') && dataConfig['orderby'] != null) {
        endPoint += "&orderby=${dataConfig["orderby"]}";
      }

      if (dataConfig.containsKey('order') && dataConfig['order'] != null) {
        endPoint += "&order=${dataConfig["order"]}";
      }

      if (dataConfig.containsKey('page')) {
        endPoint += "&page=${dataConfig["page"]}";
      }
      if (dataConfig.containsKey('limit')) {
        endPoint += "&per_page=${dataConfig["limit"] ?? apiPageSize}";
      }
  ...
  Future<List<Product>?> fetchProductsByCategory({
    ...
    try {
      ...
      if (enableBoostEngine) {
        return boostProductsByCategory(
      ...
      if (include == null || include.isEmpty) {
        ...
        if (attribute != null && attribute.toString().isNotEmpty) {
          endPoint += '&attribute=$attribute';
        }

        if (attributeTerm != null && attributeTerm.toString().isNotEmpty) {
          endPoint += '&attribute_term=$attributeTerm';
        }
  ...
  Future<PagingResponse<Product>> searchProducts({
    ...
    try {
      ...
      if (kBoostEngineConfig.isOptimizeEnable) {
        var products = await boostProducts(
          category: categoryId,
          tag: tag,
          attribute: attribute,
